...
# Check that created user (ARTIFACTORY_USER) can actually run in the given directory
checkPermissions() {
    echo -n "Checking that user ${ARTIFACTORY_USER} has permission to run in ${ARTIFACTORY_HOME}..."
    su -s "/bin/sh" ${ARTIFACTORY_USER} -c "ls ${ARTIFACTORY_HOME}/bin" > /dev/null 2>&1
    if [ $? -ne 0 ]; then
        errorArtHome "User ${ARTIFACTORY_USER} does not have proper permissions to run in ${ARTIFACTORY_HOME}"
    fi
    echo " DONE"
}
...
createArtDefault() {
    ...
    if [ -e ${artDefaultFile} ]; then
        ...
        sed --in-place -e "
            ...
            s,export ARTIFACTORY_PID=.*,export ARTIFACTORY_PID=/var/run/artifactory.pid,g;" ${artDefaultFile} || \
...
echo
echo "Installing artifactory as a service that will run as user ${ARTIFACTORY_USER}${ARTIFACTORY_GROUP:+ and group $ARTIFACTORY_GROUP}"
