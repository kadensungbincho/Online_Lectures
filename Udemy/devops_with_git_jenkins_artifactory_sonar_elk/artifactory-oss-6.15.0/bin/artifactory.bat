...
set JAVA_OPTIONS=-server -Xms512m -Xmx4g -Xss256k -XX:+UseG1GC
set JOIN_PATHS=%ARTIFACTORY_HOME%

set CATALINA_OPTS=%JAVA_OPTIONS% -Djfrog.join.key.paths=%JOIN_PATHS% -Dartifactory.home="%ARTIFACTORY_HOME%" -Dfile.encoding=UTF8 -Djruby.compile.invokedynamic=false -Djruby.bytecode.version=1.8 -Dartdist=zip -Dorg.apache.tomcat.util.buf.UDecoder.ALLOW_ENCODED_SLASH=true
