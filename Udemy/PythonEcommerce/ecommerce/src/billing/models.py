...
import stripe 
stripe.api_key = "sk_test_n8yvgO1bnpBxKEbudSxfR4vO"
...
def billing_profile_created_receiver(sender, instance, *args, **kwargs):
    if not instance.customer_id and instance.email:
        print("ACTUAL REQUEST API Send to stripe/braintree")
...
class CardManger(models.Manager):
...
class ChargeManager(models.Manager):
    def do(self, billing_profile, order_obj, card=None): # Charge.objects.do()
        ...
        c = stripe.Charge.create(
            amount = int(order_obj.total * 100), # 39.19 -> 3919 paid 39.19 
            currency = "usd",
