...
class Assembler {
  ...
  feed(data) {
    ...
    // Because the response protocol is either "empty array + response" or "error info",
    // this always works, as long as there's more than 1 byte. (A 1 byte response is never
    // valid on its own.)
    if(this._buffer.length > 1) {
      let needed = CborSkipper.skipCbor(this._buffer.buf, this._buffer._pos, this._buffer._end);
      if(needed === 0) {
